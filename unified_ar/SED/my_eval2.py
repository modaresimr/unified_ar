...
def get_single_result_df(groundtruth,predictions,meta_df=None,psdsf=None,debug=0):
    ...
    def calcs(metric):
        ...
        # df['f1']=2*df['precision']*df['recall']/(df['precision']+df['recall'])    
        return df
    events_metric = SED.evaluation_measures.event_based_evaluation_df(groundtruth, predictions, t_collar=0.500,percentage_of_length=0.5)
