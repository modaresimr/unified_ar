    ...
    while True:
        ...
        if last_segment is not None:
            ...
            if(len(values_to_consider)==0):
                values_to_consider.append(seg_start)
...
def compute_event_scores(segments):
    ...
    for i,ik in enumerate(ground_truth_indexes):
        for j,jk in enumerate(detected_indexes):
...
def eval_events(ground_truth_events, detected_events, evaluation_start=None, evaluation_end=None):
    ...
    standard_score_statistics=0 # _get_standard_event_metrics(ground_truth_events, detected_events, gt_scores, detection_scores)
