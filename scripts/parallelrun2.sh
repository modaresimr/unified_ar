...
data='5'
comment='ali'
parallel --eta  -j 7 'echo run {1} on data {2} and segm {3} with param {4} feat={5}; TF_CPP_MIN_LOG_LEVEL=3 GOTOBLAS_MAIN_FREE=1 CUDA_VISIBLE_DEVICES="" python main.py -c {1} -d {2} -s {3} -sp {4} -f {5}  -st 0 > $logdir/c={1}-d={2}-s={3}-sp-{4}-f={5}-st=0 2> >(tee -a $logdir/c={1}-d={2}-s={3}-f={5}-st=0 >&2)' ::: $comment :::  $data  ::: $seg ::: $seg_param ::: $feat
